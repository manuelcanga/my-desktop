...
abstract class Widget {
    ...
    /**
     * Render( parse a view and display it ) a widget according to view.
     *
    ...
    private function get_common_filters(): array
    {
        ...
        $filters[ 'image_url' ] = static function ( string $image_file ): string {
            return PLUGIN_URL . self::IMAGE_PATH . $image_file;
        };

        $filters[ 'base_url' ] = static function ( string $widget_url ): string {
            if ( '/' === $widget_url[ 0 ] ) {
        ...
        $filters[ 'tokenize_url' ] = static function ( string $url ) use ( $generated_token ): string {
            // Only internal urls have tokens
            if ( 0 !== strpos( $url, SITE_PATH . '/wp-admin' ) && 0 !== strpos( $url, MY_DESKTOP_URL )) {
                return $url;
            }
