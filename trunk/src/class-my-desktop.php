...
class My_Desktop {
	...
	/**
	 * Check if user is trying my_desktop without being logged.
	 *
	 * @return void
	 */
	private function check_access_to_my_desktop_without_login(): void
	{
		...
		if ( !is_user_logged_in() && get_self_link() === admin_url( 'my-desktop' ) ) {
			...
			$this->redirecto_to_login( sanitize_user( $_REQUEST[ 'redirect_to' ] ?? '' ) );
		}
	}
	 ...
	 *
	 * @param string $redirect_to It's a param used in login page.
	 *
	 * @return void
	 */
	private function redirecto_to_login( string $redirect_to = '' ): void
	{
