...
jQuery.window = function (options) {
    return jQuery.Window.getInstance(null, options);
};

// create window on caller element
jQuery.fn.window = function (options) {
    return jQuery.Window.getInstance(jQuery(this), options);
}
...
jQuery.Window = (function () {
    ...
    var ICON_WH = 22;               // window icon button width/height, in pixels. check "window_icon_button" style in css
    ...
    function pushMinWindow(parent, wnd) {
        if (setting.dockArea != null) {
            parent = jQuery(setting.dockArea);
        }
        if (parent != null) {
    ...
    function popMinWindow(parent, wnd) {
        var doAdjust = false;
        parent = (setting.dockArea != null) ? jQuery(setting.dockArea) : parent;
    ...
    function constructor(caller, options) {
        ...
        var options = jQuery.extend({
            ...
            height: 700,                  // [number:300] window height
            minWidth: 250,                // [number:200] the minimum width, if -1 means no checking
            minHeight: 250,               // [number:150] the minimum height, if -1 means no checking
            maxWidth: 2500,                // [number:800] the maximum width, if -1 means no checking
            maxHeight: 2500,               // [number:600] the maximum height, if -1 means no checking
            ...
            withinBrowserWindow: true,   // [boolean:false] to limit window only can be dragged within browser window. this attribute only works when checkBoundary is true and caller is null.
        ...
        function initialize(instance) {
            ...
            // onOpen call back
            if (jQuery.isFunction(options.onOpen)) {
            ...
            jQuery(".window_icon_button").dblclick(function () {
                return false;
            });
            ...
            if (options.url != null && jQuery.trim(options.url) != "") {
                // iframe starting call back
                if (jQuery.isFunction(options.onIframeStart)) {
            ...
            if (options.resizable) {
                ...
                // set boundary if got opotions
                if (options.checkBoundary) {
                    var caller_height = caller.height() - 30;
                    if (maxHeight > caller_height) {
                        maxHeight = caller_height;
                    }
                    var caller_width = caller.width() - 30;
                    if (maxWidth > caller_width) {
                        maxWidth = caller_width;
                    }
                ...
                if (options.maxHeight >= 0) {
                    container.resizable('option', 'maxHeight', maxHeight);
                }
        ...
        function showOverlay() {
            var overlay = jQuery("#window_overlay");
            if (overlay.get(0) == null) {
                jQuery("body").append("<div id='window_overlay'>&nbsp;</div>");
                overlay = jQuery("#window_overlay");
                    ...
                    if (btnData != null && typeof btnData == 'object') {
                        if (btnData.id != null && btnData.callback != null) { // it's a JSON object
                            ...
                            if (jQuery.isFunction(callback)) {
                                btn.click(function () {
                                    this.clickCb(jQuery(this), _this);
                                });
                            }
                        } else { // it's a html element(or wrapped with jQuery)
                            var btn = jQuery(btnData).clone(true);
        ...
        function unselect() {
            ...
            if (jQuery.isFunction(options.onUnselect)) {
                options.onUnselect();
            }
        }
        ...
        function maximize(bImmediately, bNoSaveDisplay) {
            ...
            if (bImmediately) {
                ...
                container.animate(targetCssStyle, setting.animationSpeed, 'swing', function () {
                    ...
                    // after callback
                    if (jQuery.isFunction(options.afterMaximize)) {
            ...
            // before callback
            if (jQuery.isFunction(options.onMaximize)) {
        ...
        function minimize() {
            ...
            // must add this, or it will get a bug when user mouse down the border of window panel if it is resized.
            container.addClass('no-resizable');
            container.addClass('minimize-window');
            ...
            _adjustMinimizedPos(false, function () {
                ...
                // after callback
                if (jQuery.isFunction(options.afterMinimize)) {
            ...
            container.mouseout(function () {
                jQuery(this).css("opacity", OPACITY_MINIMIZED);
            });

            // before callback
            if (jQuery.isFunction(options.onMinimize)) {
        ...
        function restore() {
            ...
            container.animate(targetCssStyle, setting.animationSpeed, 'swing', function () {
                ...
                // after callback
                if (jQuery.isFunction(options.afterCascade)) {
                    options.afterCascade(_this);
                }
            });

            // before callback
            if (jQuery.isFunction(options.onCascade)) {
        ...
        function close(quiet) {
            // do callback
            if (!quiet && jQuery.isFunction(options.onClose)) {
    ...
    return { // static public methods
        ...
        prepare: function (custSetting) { // initialize with customerized static setting attributes
            jQuery.extend(setting, custSetting);
            minWinData.long = setting.minWinLong;

            // check dock area attribute
            if (setting.dockArea != null) {
                var dArea = jQuery(setting.dockArea);
        ...
        minimizeAll: function () { // hide all created windows
            for (var i = 0, len = windowStorage.length; i < len; i++) {
...
// alias methods
jQuery.window.getVersion = jQuery.Window.getVersion;
jQuery.window.prepare = jQuery.Window.prepare;
jQuery.window.closeAll = jQuery.Window.closeAll;
jQuery.window.hideAll = jQuery.Window.hideAll;
jQuery.window.showAll = jQuery.Window.showAll;
jQuery.window.getAll = jQuery.Window.getAll;
jQuery.window.getWindow = jQuery.Window.getWindow;
jQuery.window.getSelectedWindow = jQuery.Window.getSelectedWindow;
